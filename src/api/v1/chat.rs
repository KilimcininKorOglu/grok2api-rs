...
fn validate_request(req: &ChatCompletionRequest) -> Result<(), ApiError> {
    if !ModelService::valid(&req.model) {
        return Err(ApiError::not_found(format!("The model `{}` does not exist or you do not have access to it.", req.model)).with_param("model").with_code("model_not_found"));
    }

    for (idx, msg) in req.messages.iter().enumerate() {
        let role = msg.get("role").and_then(|v| v.as_str()).unwrap_or("");
        if !VALID_ROLES.contains(&role) {
            return Err(ApiError::invalid_request(format!("role must be one of {:?}", VALID_ROLES)).with_param(format!("messages.{idx}.role")));
        }
        let content = msg.get("content");
        if content.is_none() {
            return Err(ApiError::invalid_request("Message content cannot be empty").with_param(format!("messages.{idx}.content")));
        }
        ...
        if let Some(text) = content.as_str() {
            ...
            if arr.is_empty() {
                return Err(ApiError::invalid_request("Message content cannot be an empty array").with_param(format!("messages.{idx}.content")));
            }
            for (bidx, block) in arr.iter().enumerate() {
                if block.is_null() {
                    return Err(ApiError::invalid_request("Content block cannot be empty").with_param(format!("messages.{idx}.content.{bidx}")));
                }
                let block_type = block.get("type").and_then(|v| v.as_str()).unwrap_or("");
                if block_type.is_empty() {
                    return Err(ApiError::invalid_request("Content block 'type' cannot be empty").with_param(format!("messages.{idx}.content.{bidx}.type")));
                }
                if role == "user" {
                    if !USER_CONTENT_TYPES.contains(&block_type) {
                        return Err(ApiError::invalid_request(format!("Invalid content block type: '{block_type}'")).with_param(format!("messages.{idx}.content.{bidx}.type")));
                    }
                } else if block_type != "text" {
                    return Err(ApiError::invalid_request(format!("The `{}` role only supports 'text' type, got '{}'", role, block_type)).with_param(format!("messages.{idx}.content.{bidx}.type")));
                }
                ...
                match block_type {
                    "text" => {
                        ...
                        if text.trim().is_empty() {
                            return Err(ApiError::invalid_request("Text content cannot be empty").with_param(format!("messages.{idx}.content.{bidx}.text")));
                        }
                    }
                    "image_url" => {
                        let image_url = block.get("image_url");
                        let url = image_url.and_then(|v| v.get("url")).and_then(|v| v.as_str());
                        if url.is_none() {
                            return Err(ApiError::invalid_request("image_url must have a 'url' field").with_param(format!("messages.{idx}.content.{bidx}.image_url")));
                        }
                    }
                    "input_audio" => {
                        let data = block.get("input_audio").and_then(|v| v.get("data")).and_then(|v| v.as_str());
                        if data.is_none() {
                            return Err(ApiError::invalid_request("input_audio must have a 'data' field").with_param(format!("messages.{idx}.content.{bidx}.input_audio")));
                        }
                    }
                    "file" => {
                        let file = block.get("file");
                        let url = file.and_then(|v| v.get("url").and_then(|v| v.as_str()).or_else(|| v.get("data").and_then(|v| v.as_str())));
                        if url.is_none() {
                            return Err(ApiError::invalid_request("file must have a 'url' or 'data' field").with_param(format!("messages.{idx}.content.{bidx}.file")));
                        }
                    }
                    _ => {}
                }
...
async fn chat_completions(headers: HeaderMap, Json(req): Json<ChatCompletionRequest>) -> Result<Response, ApiError> {
    ...
    let model_info = ModelService::get(&req.model).ok_or_else(|| ApiError::invalid_request("Invalid model"))?;
    if model_info.is_video {
        ...
        let result = ChatService::completions(&req.model, req.messages.clone(), req.stream, req.thinking.clone()).await?;
        match result {
